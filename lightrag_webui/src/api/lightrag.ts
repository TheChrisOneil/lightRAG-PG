...
import { useCustomSettingsStore } from '@/stores/settings_tnc' // TechNexusClarity custom state
 ...
 * └────────────────────────────────────────────────────────┘
 *
 * This module has been modified to inject a custom header
 * `X-Namespace` into all outgoing axios requests. The value
 * is pulled from the TechNexusClarity custom store: `replySettings.namespace`.
 *
 * This supports multi-tenant routing and does not interfere
 * with upstream configuration. All custom logic is encapsulated
 * between clearly marked "TechNexusClarity Custom" comment blocks.
 */
...
axiosInstance.interceptors.request.use((config) => {
  ...
  const namespace = useCustomSettingsStore.getState().replySettings.namespace
  if (namespace) {
