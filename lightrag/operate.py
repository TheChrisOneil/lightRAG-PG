...
async def extract_entities(
    ...
    logger.info(f"Example number: {example_number}")
    ...
    logger.info("Getting graph database lock...")
    ...
    async def _user_llm_func_with_cache(
        ...
        if enable_llm_cache_for_entity_extract and llm_response_cache:
            ...
            logger.debug(f"Calling LLM with input (truncated): {input_text[:5]}")
            if history_messages:
    ...
    async def _process_extraction_result(
        ...
        for record in records:
            logger.debug(f"Raw record before regex: {record}")
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        logger.debug(f"Raw LLM output for chunk {chunk_key}: {final_result}")
        ...
        for now_glean_index in range(entity_extract_max_gleaning):
            ...
            logger.debug(f"Gleaning output [{now_glean_index}] for chunk {chunk_key}: {glean_result}")
