 ...
 * └─────────────────────────────────────────────┘
 Refactored by Tech Nexus Clarity and injects the Rag instance and namespace
 into the various routes.
...
def create_document_routes(
    doc_manager: DocumentManager, api_key: Optional[str] = None # TNC remove rag
):
    ...
    async def upload_to_input_dir(
        ...
        rag: Any = Depends(get_rag),  #TNC 
        namespace: Optional[str] = Depends(get_resolved_namespace), #TNC
    ):
        """
        ...
        try:
            ...
            # ------------------------TNC------------------------------
            # Generate a unique filename with the namespace prefix
            prefixed_filename = f"{namespace}_{file.filename}"
            file_path = doc_manager.input_dir / prefixed_filename
            # --------------------------------------------------------
    ...
    async def insert_file(
        ...
        try:
            #------------------------------------------------------
            # Generate a unique filename with the namespace prefix
            file.filename = f"{namespace}_{file.filename}"  #TNC
            #------------------------------------------------------
            if not doc_manager.is_supported_file(file.filename):
                raise HTTPException(
                    status_code=400,
