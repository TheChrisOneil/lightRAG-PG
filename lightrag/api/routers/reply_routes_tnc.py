...
from lightrag.base_tnc import ReplyParam, CoachMessage as BaseCoachMessage, AISuggestion as BaseAISuggestion
from ascii_colors import trace_exception
from dotenv import load_dotenv
import os
from lightrag.lightrag import LightRAG 
from lightrag.lightrag_coach_reply_tnc import CoachReply
...
async def reply_text(
    ...
    try:
        ...
        coachReply = CoachReply(rag)
        base_coach_msg = await coachReply.acoach_reply(
...
async def query_text_stream(
    ...
    try:
        ...
        coachReploy = CoachReply(rag)
        coach_msg = await coachReploy.acoach_reply(
            conversation_history=[turn.model_dump() if isinstance(turn, DialogTurn) else turn for turn in request.conversation_history],
            student_name=request.student_name,
            speaker=request.speaker,
            content=request.content,
